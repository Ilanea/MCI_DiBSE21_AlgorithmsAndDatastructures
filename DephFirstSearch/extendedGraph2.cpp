...
void Graph::depthSearch(int i, bool visited[]){
    visited[i] = true;
    std::cout << "<" << this->vertices[i] << ">" << " -> ";
    std::vector<std::string> neighbours = this->neighbours(this->vertices[i]);

    for(std::vector<std::string>::iterator adj = neighbours.begin(); adj != neighbours.end(); ++adj){
        int j = this->resolveVertex(*adj);
        if(!visited[j]){
            depthSearch(j, visited);
        }
    }
}
...
int Graph::getEdgeWeight(std::string sourceVertex, std::string targetVertex){
    int source = this->resolveVertex(sourceVertex);
    int target = this->resolveVertex(targetVertex);

    if(this->adjacencyMatrix[source][target] != 0 || this->adjacencyMatrix[target][source] != 0) {
